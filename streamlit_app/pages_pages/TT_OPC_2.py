...
def render():
    ...
    for m in TT_MODULES:
        with st.expander(f"{m['icon']} **{m['name']}** ({m['nameEn']}) — {m['theory']}", expanded=False):
            ...
            st.markdown('**📊 评估标准**：')
            eval_criteria = {'tt-blindbox': ['✅ 综合评分 ≥ 70 分为可投', '✅ TRL 评估准确性 ≥ 80%', '✅ SWOT 四象限完整度 100%', '✅ AI 增强流式输出延迟 < 5s'], 'tt-fedmatch': ['✅ Top-5 匹配命中率 ≥ 75%', '✅ 跨校数据不出域（联邦学习 R² ≥ 95%）', '✅ 单次匹配 < 10s'], 'tt-knowledge': ['✅ 知识图谱节点数 ≥ 10K', '✅ 论文-产品路径可视化完整度 90%'], 'tt-radar': ['✅ 技术生命周期识别准确率 ≥ 70%', '✅ 替代时机窗口预测误差 < 6 个月'], 'tt-translate': ['✅ 三版本自动生成准确率 ≥ 85%', '✅ SSE 流式输出延迟 < 3s/version'], 'tt-triple': ['✅ 三角色分析完整度 100%', '✅ 协同流式输出延迟 < 5s'], 'tt-social': ['✅ 网络效应飞轮验证度 ≥ 80%', '✅ 社区活跃用户月增长 ≥ 15%'], 'tt-thermo': ['✅ AI 渗透率测算误差 < 5%', '✅ 区域创新热度地图更新频率 < 7d']}
